...
export const set = (obj: any, path: Route, value: any) => {
  path.reduce((val, key, i) => {
    const child = val[key]
    if (!child || (typeof child !== 'object' && typeof child !== 'function')) val[key] = {}
    if (i === path.length - 1) val[key] = value
    return val[key]
  }, obj)
  return value
}

export const deepKeys = (parent: unknown, isLeaf = (val: unknown) => false) => {
